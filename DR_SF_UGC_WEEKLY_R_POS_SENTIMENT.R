...
########################################
#system("chmod -R 755 /home/akokoti/automated_jobs")
#setwd("/home/akokoti/automated_jobs")
require("data.table")
require("dplyr")

reviews=dbGetQuery(con,'SELECT a.MODERATION_STATUS_DESC,a.REVIEW_ID,a.PRODUCT,a.REVIEW_TITLE,a.REVIEW_TEXT,a.REVIEW_RATING_NBR FROM ACTIVITY.ACTIVITY.PRODUCT_REVIEW a LEFT JOIN DIGITAL_RETAIL_ANALYSIS.DWADMIN.DR_SF_UGC_PRODUCT_REVIEW_SENTIMENT_SCORE b ON a.REVIEW_ID=b.REVIEW_ID WHERE b.REVIEW_ID is null')
...
bing_sentiment=dbGetQuery(con,'SELECT * FROM DIGITAL_RETAIL_ANALYSIS.DWADMIN.AK_BING_SENTIMENT_DICTIONARY')
...
dbSendQuery(con,'DROP TABLE IF EXISTS DIGITAL_RETAIL_ANALYSIS.DWADMIN.DR_SF_UGC_PRODUCT_REVIEW_POS_TEMP')
dbWriteTable(
  con,  #connection name as defined by dbConnect(jdbcDriver, ...)
  name='"DIGITAL_RETAIL_ANALYSIS"."DWADMIN"."DR_SF_UGC_PRODUCT_REVIEW_POS_TEMP"', #snowflake table name
